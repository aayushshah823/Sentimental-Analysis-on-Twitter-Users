...
def load_clean_dataset(is_train):
    # load documents
    neg = process_docs( 'Splitted_txt_sentoken/Neg' , is_train)
    pos = process_docs( 'Splitted_txt_sentoken/Pos' , is_train)
    docs = neg + pos
    # prepare labels
...
# load the model
model = load_model( '/Users/aayushshah/Documents/v6/ nGram_model.h5' )
# evaluate model on training dataset
_, acc = model.evaluate([trainX,trainX,trainX], array(trainLabels), verbose=0)
# _, acc = model.evaluate([trainX], array(trainLabels), verbose=0)
print( 'Train Accuracy: %.2f' % (acc*100))
# evaluate model on test dataset dataset
_, acc = model.evaluate([testX,testX,testX], array(testLabels), verbose=0)
print( 'Test Accuracy: %.2f' % (acc*100))

tweets = open("Test Data/Test_Tweet.txt" , "r")

with open('Test_Tweet_Sentiment_nGram_CNN.txt', 'w') as readmefile:
...
model = load_model(' nGram_model.h5')
_, acc = model.evaluate([trainX, trainX, trainX], array(trainLabels), verbose=0)
print('Train Accuracy: %.2f' % (acc*100))
...
tweet = open('Test Data/Test_Tweet.txt', 'r')
# line = tweet.readline()
# while line:
#     percent, sentiment = predict_sentiment(line, tokenizer, length, model)
#     print('Review: [%s]\nSentiment: %s (%.3f%%)' % (line, sentiment, percent*100))
#     line = lionKing.readline()
# tweet.close()


with open('Test_Tweet_Sentiments_N Gram CNN.txt', 'w') as readmefile:
